...
- **Enviar audio a otro equipo (SideBar)**: `scripts/audio-route.sh` usa `ssh` sin contraseña (llaves) y `pactl` entre `CachyOS-PC` y `CachyOS-Laptop`, así que cada equipo necesita sshd activo y la llave del otro en `~/.ssh/authorized_keys`. Con ufw activo hay que abrir además el puerto del túnel de audio en ambos: `sudo ufw allow from 192.168.0.0/16 to any port 4713 proto tcp` (si está cerrado, el botón muestra un aviso en vez de fallar en silencio). Si un equipo ya le está enviando audio al otro, el envío en sentido contrario se bloquea con una notificación (los dos túneles harían que el audio diera vueltas entre ambos).
