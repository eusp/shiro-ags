...
function loadThemes() {
    ...
    if (idx > 0)
        results.unshift(results.splice(idx, 1)[0]);
    return results;
}
function buildCssVars(data) {
    const c = (hex) => hexToRgbStr(hex);
    return `
* {
...
function hotReload(data) {
    ...
    try {
        ...
        provider.load_from_data(bytes, bytes.length);
    }
...
// ─── Wallpaper Thumbnail ──────────────────────────────────────────────────────
function WallpaperThumbnail(slug, baseColor) {
    ...
    if (GLib.file_test(pngPath, GLib.FileTest.EXISTS)) {
        ...
        try {
            ...
            const cropped = scaled.new_subpixbuf(0, 0, CARD_WIDTH, cropH);
