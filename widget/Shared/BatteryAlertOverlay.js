...
export default function BatteryAlertOverlay(gdkmonitor) {
    ...
    const overlayWindow = (_jsx(Astal.Window, { name: "battery-alert-overlay", cssClasses: ["BatteryAlertOverlay"], visible: false, gdkmonitor: gdkmonitor, anchor: TOP | BOTTOM | LEFT | RIGHT, exclusivity: Astal.Exclusivity.IGNORE, keymode: Astal.Keymode.ON_DEMAND, application: app, layer: Astal.Layer.OVERLAY, children: card }));
    overlayWindow.set_default_size(monWidth, monHeight);
    closeBtn.connect("clicked", () => {
        overlayWindow.visible = false;
    });
    battery.connect("low-battery", (_self, pct, critical) => {
